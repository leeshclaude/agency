    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl" style={{ fontFamily: 'Josefin Sans, sans-serif', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#2C1A22' }}>Sessions</h1>
        </div>
        {isAdmin && (
          <button
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="mt-3 text-sm" style={{ color: '#6B4A57' }}>No upcoming sessions</p>
          {isAdmin && (
            <button
              onClick={() => { setEditingSession(null); setShowModal(true) }}
...
function SessionCard({ session, isRegistered, isFull, count, isAdmin, onRegister, onUnregister, onEdit, onDelete, onViewRegistrants, expandedRegs }) {
  ...
  return (
    <div className="card overflow-hidden">
      {/* Top accent */}
      <div className="h-1" style={{ background: '#D4688A' }} />
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div>
            <h3 style={{ fontFamily: 'Josefin Sans, sans-serif', fontWeight: 700, letterSpacing: '0.10em', color: '#2C1A22' }}>{session.title}</h3>
        ...
        <div className="flex items-center gap-4 text-sm mb-3" style={{ fontFamily: 'DM Sans, sans-serif', color: '#6B4A57' }}>
          <span>📅 {dateStr}</span>
        </div>
        <div className="flex items-center gap-4 text-sm mb-4" style={{ color: '#6B4A57' }}>
          <span>🕐 {timeStr} AEST</span>
        </div>

        {session.description && (
          <p className="text-sm mb-4" style={{ fontFamily: 'DM Sans, sans-serif', color: '#6B4A57' }}>{session.description}</p>
        )}
            ...
            {expandedRegs !== null && (
              <div className="mt-3 space-y-2">
                {expandedRegs.length === 0 ? (
                  <p className="text-xs" style={{ color: '#6B4A57' }}>No one registered yet</p>
                ) : (
                  expandedRegs.map((r, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs" style={{ color: '#2C1A22' }}>
                      <span className="font-medium">{r.profiles?.full_name}</span>
                      <span style={{ color: '#6B4A57' }}>{r.profiles?.instagram_handle}</span>
                      <span style={{ color: '#6B4A57' }}>{r.profiles?.email}</span>
                    </div>
                  ))
                )}
              </div>
...
function SessionModal({ session, profileId, onClose, onSaved }) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(44,26,34,0.5)' }}
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div
        className="w-full max-w-lg mx-auto rounded-t-3xl px-5 pt-5 pb-8 space-y-4"
        style={{ background: '#FEF9FB', maxHeight: '85vh', overflowY: 'auto' }}
      >
        <div className="flex items-center justify-between mb-1">
          <h2 style={{ fontFamily: 'Josefin Sans, sans-serif', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#2C1A22' }}>
